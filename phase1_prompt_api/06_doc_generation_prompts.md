...
- 硬件：MacBook Air M5，32GB 统一内存，1TB SSD，macOS。
- 环境隔离：只使用 Conda 虚拟环境（环境名为 `cxllm`），Python 3.11。绝不使用 venv。
- 开源目标：这个项目最终会提交到 GitHub 作为我的个人作品集，供他人克隆学习。代码结构必须高度规范，防呆设计要足。
- 工具链原则：必须使用行业最新版的命令和工具（优先使用针对 Apple Silicon Metal 加速的底层优化参数）。

# Task Requirements
请为我生成一份详细的 `WeekX_runbook.md`，必须包含以下核心章节，且语气要专业、硬核，充满工程化洞察。

**核心红线要求**：
1. **零省略原则**：坚决不允许省略任何步骤。必须提供所有具体的终端执行命令（包括 `mkdir -p` 创建文件夹、`touch` 创建空文件）。
2. **源码直出**：所有涉及的 `.py`、`.sh` 以及配置文件的代码，必须使用 `cat << 'EOF' > filename` 的 Bash 语法格式完整输出，确保我复制进终端回车就能一键生成源码文件。

## 1. 工程化目录架构
- 生成一个符合现代 Python/AI 项目规范的目录树（包含 `configs/`, `scripts/`, `src/`, `tests/`, `docs/` 等）。
- 提供一键初始化该目录树的完整 Shell 命令。
- 用一句话精准解释每个核心目录的工程作用。

## 2. 依赖安装与最新工具链配置
- 给出在 `cxllm` 环境下的极速包安装命令。如果涉及 Apple Metal 加速（如 MPS 版本的 PyTorch 或 MLX 框架），必须提供精准的安装命令及避坑指南。
- **网络与模型下载约束**：必须配置国内镜像（如注入 `export HF_ENDPOINT=https://hf-mirror.com` 环境变量）。使用最新工具进行极速下载，确保在国内网络环境下命令绝对跑通。

## 3. 分终端执行与测试流程（Debug 视角）
- 按照真实的开发顺序，教我如何开多个终端分布调试（例如终端 1 跑底座服务，终端 2 跑网关，终端 3 做 Curl 或业务层压测）。
- 必须包含具体的启动命令、预估的模型下载体积（需精算 32GB 内存与 1TB 固态的承载上限），以及预期应该看到的成功日志（Stdout/Stderr）。

## 4. 终极一键运行：Makefile 集成
- 因为我要开源给社区或面试官看，必须提供一个极致优雅的 `Makefile` 源码。
- 里面需要包含 `make setup` (初始化环境/下载模型)、`make run-all` (一键按顺序拉起所有服务)、`make clean` (清理缓存和无用数据) 等指令。
- 演示如何使用 `nohup` 或后台进程技巧实现在一个命令下管理多个服务，并提供优雅的 Kill 方案。

## 5. 常见坑点与硬件降维打击方案
- 针对 32GB M5 芯片、Conda 隔离环境或代理劫持可能遇到的报错（如端口冲突、OOM），给出至少 3 个提前预判的避坑指南。
- 针对从 Windows 平台迁移到 macOS 进行 AI 开发的常见痛点，给出特别提示（如路径斜杠、进程守护差异等）。

## 6. 面试深度解析
- 针对本周的实战主题，给出 3 道资深 AI Infra 工程师级别的面试题以及核心答题思路，要求包含对底层硬件（如显存带宽、算子调度）或数据流转视角的深度思考。

# Output Style
- 输出格式必须是完整的 Markdown，方便我直接复制保存为 `runbook.md`。
- 不要废话，不要包含任何“你好，我是AI”的开场白，直接输出硬核的文档内容。
```
