...
通过矩形框 Prompt 调用 SAM2 进行交互式分割。
输出 mask 遮罩图 + 叠加 overlay + 质量报告 JSON。

数据流：
  输入图片 → set_image() → encode image feature
    ↓
  box (x1,y1,x2,y2) → predict(box=input_box) → masks + scores
    ↓
  best_mask → 保存 PNG + 叠加 overlay + 质量报告

使用方式：
  python scripts/03_segment_box.py \
...
def main():
    ...
    # ── 5. Box Prompt 推理 ─────────────────────────────────
    # Box 格式: np.array([[x_min, y_min, x_max, y_max]])
