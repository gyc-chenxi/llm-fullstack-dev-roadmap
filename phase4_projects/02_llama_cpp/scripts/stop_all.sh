...
# ──────────────────────────────────────────────────────────────────────────────
# stop_all.sh — graceful shutdown of all AI Gateway services.
# ──────────────────────────────────────────────────────────────────────────────
...
pkill -f "llama-server.*qwen2.5" 2>/dev/null && {
    log_info "Stopped running llama-server processes"; STOPPED=$((STOPPED + 1));
} || true

pkill -f "uvicorn gateway.app" 2>/dev/null && {
    log_info "Stopped running gateway processes"; STOPPED=$((STOPPED + 1));
} || true
